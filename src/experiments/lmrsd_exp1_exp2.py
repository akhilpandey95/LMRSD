...
def process_prompt(raw_text, task="idea"):
    ...
    if task == "idea":
        ...
        user_prompt = """**Task:** You are given Paper title, abstract and keywords of a scientific
...
async def process_batch(client, batch, model, semaphore, content_choice="idea"):
    """
...
if __name__ == "__main__":
    ...
    logger.info(f"LMRSD EXP1 dataset length: {len(dataset)}")
    logger.info("----------------------------------")
